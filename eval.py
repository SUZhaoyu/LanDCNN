...
tf.app.flags.DEFINE_string('task_name', 'building','')
...
mean = np.load(dir_concat(FLAGS.HOME, ['dataset', FLAGS.dataset_name, 'training_mean.npy']))
std = np.load(dir_concat(FLAGS.HOME, ['dataset', FLAGS.dataset_name, 'training_std.npy']))
            ...
            for row_id in tqdm(arange(0, rows-1, kernel_size)):
                for col_id in arange(0, cols-1, kernel_size):
                    ...
                    if FLAGS.loss == 'dice':
                        ...
                        scars_map = map[0, start_margin:start_margin + row_step_size, start_margin:start_margin + col_step_size, 1] > 0.5
